...
def _weighted_lsr(
    ...
    for _ in range(max_iter):
        ...
        for l in range(m):
            ...
            # Suffix log-sum-exp for numerical stability
            suffix_theta = theta[perm]
            # suffix_lse[p] = log Σ_{q=p}^{n-1} exp(θ_{perm[q]})
            suffix_lse = np.zeros(n)
            suffix_lse[n - 1] = suffix_theta[n - 1]
            for p in range(n - 2, -1, -1):
                a, b_ = suffix_theta[p], suffix_lse[p + 1]
                suffix_lse[p] = max(a, b_) + np.log(1 + np.exp(-abs(a - b_)))
 
            # For each position p (0..n-2), the winner is perm[p]
            # and the denominator set includes positions p..n-1
            for p in range(n - 1):
                denom = np.exp(suffix_lse[p])  # Σ_{q≥p} exp(θ_q)
                if denom < 1e-300:
                    continue
                contribution = w_l / denom
                winner = perm[p]
                # For each loser at position q > p:
                for q in range(p + 1, n):
                    loser = perm[q]
                    # Transition: loser → winner (loser "loses" to winner)
                    M[loser, winner] += contribution
        ...
        p_stat = np.ones(n) / n
        for _ in range(200):
            p_new = p_stat @ M_stoch
            p_new = np.maximum(p_new, 1e-300)
            p_new /= p_new.sum()
            if np.linalg.norm(p_new - p_stat, 1) < 1e-10:
                break
            p_stat = p_new
        p_stat = p_new
...
class LearnPlackettLuceMixture:
    """
    Learn a mixture of K Plackett-Luce models from a population of permutations.

    Uses the Spectral EM algorithm (EM-LSR) from Nguyen & Zhang (2023):
      - Phase 1: Spectral initialization via SVD + k-means + least squares
      - Phase 2: EM with exact weighted log-likelihood maximization (LSR)

    Parameters are stored in log-utility (θ) scale internally.
    The returned 'weights' key gives softmax(θ) for each component,
    matching the interface of LearnPlackettLuce.
    """
    ...
    def __call__(
        ...
        """
        Learn mixture of Plackett-Luce models from a population of permutations.

        Args:
            generation:        Current generation number (passed through)
            n_vars:            Number of items (permutation length)
            cardinality:       Not used (kept for interface compatibility)
            selected_pop:      (m, n) population of permutations (0-indexed)
            selected_fitness:  (m,) fitness values (not used in learning)
            n_components:      K, number of mixture components
            max_em_iter:       Maximum EM outer iterations
            tol_em:            Convergence threshold on mixing weights L1 change
            max_iter_lsr:      Max inner iterations for Weighted LSR (M-step)
            tol_lsr:           Convergence threshold inside LSR
            use_spectral_init: If True (recommended), use spectral initialization.
                               If False, use random initialization.
            random_state:      Random seed for reproducibility

        Returns:
            Model dictionary:
                - weights_per_component: list of K arrays of length n_vars,
                  each is softmax(θ^k), i.e. the PL preference weights for
                  component k (positive, sum to 1 within each component)
                - mixing_weights: (K,) array β, the mixing proportions
                - theta:          (n, K) log-utility parameters
                - n_components:   K
                - model_type:     "plackett_luce_mixture"
                - weights:        weights of the dominant component
                  (for drop-in compatibility with single-PL interface)
        """
        data = np.asarray(selected_pop, dtype=int)
        m, n = data.shape
        K = n_components
        ...
        # ----------------------------------------------------------------
        if use_spectral_init and m >= K * 2:
            theta, _ = _spectral_init(data, K, random_state=random_state)
        else:
            # Random initialization: sample θ from N(0, 1), normalize
        ...
        for em_iter in range(max_em_iter):
            ...
            # E-step
            q = _e_step(data, theta, beta)  # (m, K)

            # Update mixing weights: β^k = (1/m) Σₗ q^k_l
            beta = q.mean(axis=0)
            beta = np.maximum(beta, 1e-9)
            beta /= beta.sum()

            # M-step (Weighted LSR per component)
            theta = _m_step(data, q, theta, max_iter_lsr, tol_lsr)  # (n, K)

            # Check convergence on mixing weights
            if np.linalg.norm(beta - beta_old, 1) < tol_em:
                break

        # ----------------------------------------------------------------
        # Package results
        # ----------------------------------------------------------------
        # Convert log-utilities to probability weights via softmax
        ...
        # Dominant component (highest mixing weight) for drop-in compatibility
        ...
        return {
            ...
            "n_components": K,
            "model_type": "plackett_luce_mixture",
            # Drop-in compatibility with single-PL interface:
            "weights": weights_per_component[dominant_k],
        }


def learn_plackett_luce_mixture(
    generation: int,
    n_vars: int,
    cardinality: np.ndarray,
    selected_pop: np.ndarray,
    ...
    **params,
    ...
    """
    Convenience function to learn a mixture of Plackett-Luce models.

    See LearnPlackettLuceMixture for parameter details.
    """
    learner = LearnPlackettLuceMixture()
    return learner(
