...
using the Luce Spectral Ranking (LSR) / MM algorithm.
...
class LearnPlackettLuce:
    ...
    def __call__(
        ...
        Uses the MM (Minorization-Maximization) algorithm, also known as the
        Hunter algorithm, to find the MLE of the weight vector.

        Args:
            generation: Current generation number
            n_vars: Number of variables (permutation length)
            cardinality: Not used for permutations
            selected_pop: Selected population of permutations, shape (K, n)
        ...
        Returns:
            Model dictionary containing:
                - weights: Learned weight vector of length n_vars (normalized, sums to 1)
                - model_type: "plackett_luce"
        """
        data = np.array(selected_pop, dtype=int)
        K, n = data.shape

        weights = np.ones(n, dtype=float) / n

        # Win counts: item i wins once for each position it occupies except last
        ...
        for _ in range(max_iter):
            ...
            for k in range(K):
                perm = data[k]
                # suffix_sums[p] = sum of weights of items in positions p..n-1
                ...
                # cum_inv_suffix_sums[p] = sum_{q=0}^{p} inv_suffix_sums[q]
                cum_inv = np.cumsum(inv_suffix_sums)

                for p in range(n):
                    denominators[data[k, p]] += cum_inv[p]
...
def learn_plackett_luce(
    ...
    **params,
    ...
    """
    Convenience function to learn Plackett-Luce model.

    See LearnPlackettLuce for parameter details.
    """
